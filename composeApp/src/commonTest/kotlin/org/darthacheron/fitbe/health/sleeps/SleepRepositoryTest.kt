...
class SleepRepositoryTest {
    ...
    @Test
    fun `getSleepsBetween returns empty list when no data exists`() =
        runTest {
    ...
    @Test
    fun `getSleepsBetween returns single sleep that is within one day`() =
        runTest {
            // --- GIVEN ---
    ...
    @Test
    fun `getSleepsBetween splits a sleep session that crosses midnight`() =
        runTest {
            // --- GIVEN ---
            val sleepStart = LocalDateTime(2025, 10, 15, 22, 0).toInstant(germanTimeZone) // 10 PM on 15th
    ...
    @Test
    fun `getSleepsBetween handles sleep spanning multiple days`() =
        runTest {
            // --- GIVEN ---
            val sleepStart = LocalDateTime(2025, 11, 10, 21, 0).toInstant(germanTimeZone) // 9 PM on 10th
    ...
    @Test
    fun `getSleepsBetween correctly clips sleep starting before range`() =
        runTest {
            // --- GIVEN ---
            val sleepStart =
                LocalDateTime(
    ...
    @Test
    fun `getSleepsBetween correctly clips sleep ending after range`() =
        runTest {
            // --- GIVEN ---
    ...
    @Test
    fun `getSleepsBetween EDGE CASE handles Daylight Saving Time change`() =
        runTest {
            // In 2025, Germany falls back from CEST (UTC+2) to CET (UTC+1) on Oct 26 at 3:00 AM.
            // A day has 25 hours.
            val sleepStart = LocalDateTime(2025, 10, 25, 22, 0).toInstant(germanTimeZone) // 10 PM on 25th

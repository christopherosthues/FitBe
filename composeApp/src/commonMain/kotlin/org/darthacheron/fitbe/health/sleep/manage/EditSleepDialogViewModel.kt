...
class EditSleepDialogViewModel(
    private val settingsRepository: SettingsRepository,
    private val sleepRepository: SleepRepository
) : DialogViewModel<EditSleepDialogUiState>() {
    override val uiState = MutableStateFlow(EditSleepDialogUiState())

    override fun dismissDialog() {
        uiState.update { EditSleepDialogUiState() }
    }

    fun init(id: Uuid) {
        viewModelScope.launch {
            val sleep = sleepRepository.getSleep(id)
            ...
            uiState.update {
                val start = sleep.start.toLocalDateTime(TimeZone.currentSystemDefault())
                val end = sleep.end.toLocalDateTime(TimeZone.currentSystemDefault())
                it.copy(
                    id = id,
                    startDateTime = LocalDateTime(start.date, start.time),
                    endDateTime = LocalDateTime(end.date, end.time)
                )
            }
        }
    }
